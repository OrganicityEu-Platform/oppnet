...
- The packet sent to the OppNet application must be in the following format: {"type": 102, "data": []}, inside the array defined by field "data", each element is formatted as: {"path": a unique device id, "data": data content, "delay": 0, "id": data packet id}, then the application is able to correctly parse the packet the sensor sends to it.
